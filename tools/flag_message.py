...
class FlagEmailTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            try:
                # Update email flag
                result = self._update_email_flag(access_token, email_id, flag_status, due_date_days, flag_message)
    ...
    def _update_email_flag(
        self, access_token: str, email_id: str, flag_status: str, due_date_days: int, flag_message: str
    ):
        """
        ...
        try:
            ...
            if flag_status == "flagged" and due_date_days > 0:
                ...
                flag_data["dueDateTime"] = {"dateTime": due_date.isoformat() + "Z", "timeZone": "UTC"}

                # Set start date to today
                start_date = datetime.utcnow()
                flag_data["startDateTime"] = {"dateTime": start_date.isoformat() + "Z", "timeZone": "UTC"}

            # Build the complete update payload
            update_data = {"flag": flag_data}
            ...
            get_response = requests.get(
                f"{url}?$select=id,subject,flag,categories,sender,receivedDateTime", headers=headers, timeout=30
            )
    ...
    def _extract_sender_info(self, sender_obj: dict) -> dict:
        ...
        return {"name": email_address.get("name", "Unknown"), "email": email_address.get("address", "Unknown")}
