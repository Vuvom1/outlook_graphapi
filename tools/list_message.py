...
class ListEmailsTool(Tool):
    ...
    def _get_emails(self, access_token: str, limit: int, folder: str, search_query: str, include_body: bool):
        """
        ...
        try:
            ...
            params = {"$top": limit, "$select": ",".join(select_fields)}
    ...
    def _format_email(self, msg: dict, include_body: bool = False) -> dict:
        ...
        # Get recipients
        to_recipients = [self._extract_email_address(recipient) for recipient in msg.get("toRecipients", [])]

        cc_recipients = [self._extract_email_address(recipient) for recipient in msg.get("ccRecipients", [])]
...
list_message_tool = ListEmailsTool(
    runtime=None,
    session=None,
)
