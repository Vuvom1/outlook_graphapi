...
from schemas.email_schemas import (
    ListEmailsResponse, SendEmailRequest, SendEmailResponse,
    CreateDraftRequest, CreateDraftResponse, UpdateEmailRequest, UpdateEmailResponse,
    AttachmentRequest, AttachmentResponse, PrioritizeEmailRequest, EmailResponse,
)
...
async def list_emails(
    ...
    search: Optional[str] = Query(None, description="Search query for filtering emails"),
...
async def send_draft_email(
    ...
    """
    try:
        result = await email_service.send_draft(access_token, draft_id)
...
async def prioritize_email(
    email_id: str,
    request: PrioritizeEmailRequest,
    access_token: str = Depends(get_access_token)
) -> BaseResponse:
    """
    ...
    """
    try:
        result = await email_service.prioritize_email(access_token, email_id, request.priority_level)
        
        return BaseResponse(
            message=f"Email priority set to {request.priority_level.value}"
        )
...
async def mark_email_read(
    ...
    """
    try:
        result = await email_service.mark_email_read(access_token, email_id, is_read)
